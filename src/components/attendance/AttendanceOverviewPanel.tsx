...
import type { StudentAttendanceOverview } from "@/services/attendance/studentAttendanceOverviewService";
 ...
 * (chamada pelo componente-pai, que decide QUAL aluno consultar — o
 * próprio, no caso do aluno; o filho selecionado, no caso do
 * responsável).
 */
export function AttendanceOverviewPanel({ overview }: { overview: StudentAttendanceOverview }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card className="p-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {overview.overallStatus && <AttendanceStatusBadge status={overview.overallStatus} />}
      </Card>
        ...
        {isMobile ? (
            ...
            {overview.disciplines.map((row) => (
              <div key={row.discipline.id} className="flex flex-col gap-1.5 px-4 py-3.5">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  {row.status && <AttendanceStatusBadge status={row.status} />}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <caption className="sr-only">Frequência por disciplina</caption>
              <thead>
              ...
              <tbody>
                {overview.disciplines.map((row) => (
                  <tr key={row.discipline.id} className="border-b border-line last:border-0">
                    ...
                    <td className="px-4 py-3">{row.status && <AttendanceStatusBadge status={row.status} />}</td>
                  </tr>
                ))}
              </tbody>
            </table>
